...
const capillaryGravitySpecification = composeExampleSpecification("Shallow capillary–gravity rain", {
    overrides: {
        inputRadius: {defaultValue: 10},
        inputStrength: {
            defaultValue: 0.35,
            description: "Acceleration applied by the mouse/touch brush, in metres per second squared. Successive presses alternate its sign."
        },
        initialDataFunction: {
            defaultValue: "(x, y) => [[0.0, 0.0]]",
            description: "Initially flat, motionless water. u_1 is the surface elevation in metres."
        },
        laplace: {hidden: true},
        identity: {hidden: true},
        derivative: {hidden: true},
        cubic: {hidden: true},
        noiseStrength: {hidden: true, defaultValue: 0},
        useCellularAutomatonRule: {hidden: true},
        equation: {
            defaultValue: "float rainLambdaMax = rainBaseRate * (1.0 + rainSpatialContrast) * (1.0 + rainTemporalContrast);\nfloat rainCandidateProbability = 1.0 - exp(-rainLambdaMax * width * scaleX * height * scaleY * scaleT / 4.0);\nfloat rainKick = 0.0;\nfor (int rainSlot = 0; rainSlot < 4; rainSlot++) {\n    float slot = float(rainSlot);\n    float gateU = randomValue(vec2(randSeed + 13.17 * slot, 19.91 + 7.13 * slot));\n    float locationU = randomValue(vec2(randSeed + 29.31 * slot, 37.27 + 11.73 * slot));\n    float locationV = randomValue(vec2(randSeed + 43.11 * slot, 53.93 + 17.17 * slot));\n    float acceptU = randomValue(vec2(randSeed + 61.79 * slot, 71.21 + 23.57 * slot));\n    float markU = randomValue(vec2(randSeed + 79.43 * slot, 89.17 + 29.71 * slot));\n    float rainX = offsetX + (locationU - 0.5) * width * scaleX;\n    float rainY = offsetY + (locationV - 0.5) * height * scaleY;\n    float rainLambda = max(0.0, rainBaseRate * (1.0 + rainSpatialContrast * sin(6.28318530718 * (rainX - rainDriftSpeed * t) / rainPatchLengthX) * cos(6.28318530718 * rainY / rainPatchLengthY)) * (1.0 + rainTemporalContrast * sin(6.28318530718 * t / rainPeriod)));\n    float candidateExists = 1.0 - step(rainCandidateProbability, gateU);\n    float candidateAccepted = 1.0 - step(rainLambda / max(rainLambdaMax, 0.000000000001), acceptU);\n    float candidateAtCell = (1.0 - step(0.5 * scaleX, abs(x - rainX))) * (1.0 - step(0.5 * scaleY, abs(y - rainY)));\n    float impulseMark = 1.0 + rainImpulseVariation * (2.0 * markU - 1.0);\n    rainKick += candidateExists * candidateAccepted * candidateAtCell * impulseMark;\n}\n#\nA = u_1_xx + u_1_yy;\nB = u_1_xxxx + 2.0 * u_1_xxyy + u_1_yyyy;\nu_1_tt = gravity * waterDepth * A - surfaceTensionOverDensity * waterDepth * B - waveDamping * u_1_t + rainImpulse * rainKick / max(scaleT, 0.00000001) + force;\n",
            description: "Linear shallow capillary–gravity waves. The Laplacian gives gravity waves, the negative biharmonic term gives capillary dispersion, damping removes energy, and rejection-thinned random candidates approximate a varying-intensity Poisson rain process."
        }
    },
    extraProperties: [
        {
            name: "waterDepth", displayName: "Water depth h (m)", group: "/settings/model/basic",
            type: "float", exclusiveMinimum: 0, defaultValue: 0.001,
            uniformName: "waterDepth", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Mean water depth in metres. The local model assumes the resolved wavelengths are much longer than this depth."
        },
        {
            name: "gravity", displayName: "Gravity g (m/s²)", group: "/settings/model/basic",
            type: "float", minimum: 0, defaultValue: 9.81,
            uniformName: "gravity", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Gravitational acceleration."
        },
        {
            name: "surfaceTensionOverDensity", displayName: "Surface tension / density (m³/s²)", group: "/settings/model/basic",
            type: "float", minimum: 0, defaultValue: 0.000072,
            uniformName: "surfaceTensionOverDensity", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "The capillary parameter σ/ρ; 0.000072 m³/s² is approximately clean water near room temperature."
        },
        {
            name: "waveDamping", displayName: "Wave damping (1/s)", group: "/settings/model/basic",
            type: "float", minimum: 0, defaultValue: 1.2,
            uniformName: "waveDamping", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Linear damping of surface vertical velocity."
        },
        {
            name: "rainBaseRate", displayName: "Mean rain rate (drops/m²/s)", group: "/settings/input/basic",
            type: "float", minimum: 0, defaultValue: 150,
            uniformName: "rainBaseRate", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Baseline intensity λ₀ of the spatial Poisson point process."
        },
        {
            name: "rainImpulse", displayName: "Drop velocity impulse (m/s)", group: "/settings/input/basic",
            type: "float", maximum: 0, defaultValue: -0.018,
            uniformName: "rainImpulse", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Downward surface-velocity kick delivered by one drop."
        },
        {
            name: "rainSpatialContrast", displayName: "Rain spatial contrast", group: "/settings/input/advanced",
            type: "float", minimum: 0, maximum: 0.95, defaultValue: 0.75,
            uniformName: "rainSpatialContrast", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Fractional contrast of the moving sinusoidal rain bands."
        },
        {
            name: "rainPatchLengthX", displayName: "Rain patch length X (m)", group: "/settings/input/advanced",
            type: "float", exclusiveMinimum: 0, defaultValue: 0.18,
            uniformName: "rainPatchLengthX", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Horizontal wavelength of the varying rain intensity."
        },
        {
            name: "rainPatchLengthY", displayName: "Rain patch length Y (m)", group: "/settings/input/advanced",
            type: "float", exclusiveMinimum: 0, defaultValue: 0.14,
            uniformName: "rainPatchLengthY", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Vertical wavelength of the varying rain intensity."
        },
        {
            name: "rainDriftSpeed", displayName: "Rain drift speed X (m/s)", group: "/settings/input/advanced",
            type: "float", defaultValue: 0.025,
            uniformName: "rainDriftSpeed", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Horizontal speed of the rain-intensity pattern."
        },
        {
            name: "rainTemporalContrast", displayName: "Rain temporal contrast", group: "/settings/input/advanced",
            type: "float", minimum: 0, maximum: 0.95, defaultValue: 0.5,
            uniformName: "rainTemporalContrast", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Fractional amplitude of the periodic variation in total rain rate."
        },
        {
            name: "rainPeriod", displayName: "Rain variation period (s)", group: "/settings/input/advanced",
            type: "float", exclusiveMinimum: 0, defaultValue: 1.6,
            uniformName: "rainPeriod", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Period of the temporal rain-rate modulation."
        },
        {
            name: "rainImpulseVariation", displayName: "Drop-size variation", group: "/settings/input/advanced",
            type: "float", minimum: 0, maximum: 0.95, defaultValue: 0.35,
            uniformName: "rainImpulseVariation", uniformType: "float", shaderPrograms: ["compute"],
            restartOnChange: false,
            description: "Uniform fractional variation of the impulse mark attached to each Poisson event."
        }
    ]
});

const capillaryGravitySettings = completeSettings(capillaryGravitySpecification, {
    width: 700,
    height: 700,
    scaleT: 0.00035,
    scaleX: 0.001,
    ...
    rainBaseRate: 150,
    rainImpulse: -0.018,
    rainSpatialContrast: 0.75,
    rainPatchLengthX: 0.18,
    rainPatchLengthY: 0.14,
    rainDriftSpeed: 0.025,
    rainTemporalContrast: 0.5,
    rainPeriod: 1.6,
    rainImpulseVariation: 0.35
});
...
const simulationExamples = [
    ...
    {
        id: "capillary-gravity-rain",
        name: "Shallow capillary–gravity rain",
        description: "Discrete raindrop impacts excite dispersive gravity–capillary ripples on a shallow water layer.",
        specification: capillaryGravitySpecification,
        settings: capillaryGravitySettings
    },
    {
        id: "lens-wave",
